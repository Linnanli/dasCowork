...
**状态：已实施并完成自动化验收（2026-07-11）。**

实施结果：生产 renderer 已迁移到 `app://-/index.html`，历史本地媒体使用受控的
`app://fs/@fs/...` 展示 URL，模型输入边界会在 main process 中恢复为 `file:` URL。
协议单测、历史/运行时集成测试、生产 Electron E2E、macOS `activate` 重建窗口测试和
electron-builder unpacked smoke 均已通过。
...
4. 对 `app://fs/*` 的 renderer 请求仅接受来自 `app://-` 的 `image`/`media` 子资源请求；开发模式额外允许精确匹配的 Vite origin。任意 HTTP 站点、`webview`、外部窗口、缺失 frame 的 `webRequest` 事件以及 `mainFrame`、`subFrame`、`xhr` 等其他资源类型均被取消。main process 主动调用的 `session.fetch()` 属于已有的特权主进程能力，不作为 renderer 协议来源边界的一部分。
...
- 新增纯函数 `restoreLocalMediaFileUrlsForModel(messages)`：在 `startChatStream()` 调用可注入的 `streamText` 边界前构造模型输入副本，把合法 `app://fs` 图片还原为 `pathToFileURL(resolveAppMediaPath(...))`；默认实现随后才调用 `convertToModelMessages()`。这样测试替身与真实 provider 共用同一条约束；`data:`、`blob:`、HTTP(S) 等其他协议保持既有行为，任何无法通过共享解析器的 `app:` URL 都会拒绝当前请求，不会变成本地读取或原样发往 provider。
...
- 新增 `desktop-app/tests/e2e/local-media-protocol.e2e.ts`
  ...
  - 用同一 default session 创建受控 HTTP 测试页面，真实验证 HTTP frame、主 frame 导航、非 `image`/`media` 请求和 `.txt` 无法读取协议内容；缺失 frame 的 `webRequest` 回调通过注册边界单测验证。特权 main process 的 `session.fetch()` 不属于 renderer 威胁模型，不能用它伪装成 renderer 无 frame 请求。
...
- `npm --prefix desktop-app test`：45 个测试文件、424 个测试通过。
- `npm --prefix desktop-app run lint`：通过，0 error；仍有 228 条本任务外既有 warning。
- `npm --prefix desktop-app run typecheck:node`、`typecheck:web`：通过。
- Provider `lint`、`typecheck`：通过。
- `npm --prefix desktop-app run test:e2e -- --reporter=line`：19 个测试通过，packaged 专用测试按设计跳过。
- `npm --prefix desktop-app run test:e2e:packaged`：electron-builder unpacked 可执行文件启动成功，`app.isPackaged === true`，asar 内主 JS/CSS 与本地 PNG 均通过 `app:` 加载。
- `npm --prefix desktop-app run dev`：开发 renderer 在 Vite HTTP origin 成功启动；手动结束 smoke 后进程退出码为中断码。

## 最终改动文件

- 协议与安全策略：`desktop-app/src/main/localMediaProtocol.ts`、`localMediaProtocol.test.ts`。
- 历史/模型 URL 边界：`desktop-app/src/main/conversations/localMediaUrls.ts`、`localMediaUrls.test.ts`、`ConversationApiService.ts`、`ConversationApiService.test.ts`、`codexChatRuntimeService.ts`、`codexChatRuntimeService.test.ts`；删除旧 `localImageDataUrls` 实现与测试。
- 应用接入与 CSP：`desktop-app/src/main/index.ts`、`desktop-app/src/renderer/index.html`、`desktop-app/src/renderer/src/App.test.tsx`。
- 真实应用验证：`desktop-app/tests/e2e/local-media-protocol.e2e.ts`、`packaged-local-media-smoke.e2e.ts`、`tests/e2e/support/app.ts`、`scripts/run-packaged-local-media-smoke.mjs`、`desktop-app/package.json`。

普通文件预览仍不在 `app://fs` allowlist 内；若后续需要该能力，应另开 capability URL/下载器方案。
