...
export class CodexHistoryClient
{
    ...
    {
        return this.withClient((client) =>
            client.request<TurnsPage>("thread/turns/list", stripUndefined({
                threadId,
    ...
    async forkThread(threadId: string, params: CodexThreadForkParams = {}): Promise<Thread>
    {
        return this.withClient(async (client) =>
        {
            const response = await client.request<CodexThreadForkResponse>("thread/fork", stripUndefined({
                threadId,
...
export function createCodexHistoryClient(settings: CodexHistoryClientSettings = {}): CodexHistoryClient
{
