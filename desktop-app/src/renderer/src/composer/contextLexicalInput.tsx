...
import { type ComponentPropsWithoutRef, type FC, forwardRef, useEffect, useMemo } from 'react'
...
function CursorPlugin(): null {
  ...
  useEffect(() => {
    ...
    const broadcastCursor = (pos: number): void => {
      for (const plugin of pluginRegistry.getPlugins()) {
...
function FocusPlugin({ autoFocus }: { autoFocus: boolean }): null {
  const [editor] = useLexicalComposerContext()
  const aui = useAui()
...
export const ContextLexicalInput = forwardRef<HTMLDivElement, ContextLexicalInputProps>(
  (
    {
      submitMode = 'enter',
      cancelOnEscape = true,
    ...
    const initialConfig = useMemo(
      () => ({
        ...
        nodes: [DirectiveNode],
        onError: (error: Error) => {
          console.error('[ContextLexicalInput]', error)
        }
      }),
      []
    )
...
ContextLexicalInput.displayName = 'ContextLexicalInput'

function EditablePlugin({ isDisabled }: { isDisabled: boolean }): null {
  const [editor] = useLexicalComposerContext()
  useEffect(() => {
